...
- **Bump github-actions group: actions/checkout 6→7, codecov/codecov-action 6→7, actions/create-github-app-token 3.1.1→3.2.0, EndBug/label-sync 1cd399a63f3d27e0a890cec8d1c29efad3e5db3d→9e7b9e132c14c20e0a3d2269b12f0ab39623e38b** (#65)
- **Pin `EndBug/label-sync` in `sync-labels.yml` to a Node-24 commit on upstream `main`**: the QA `sync-labels.yml` workflow used `EndBug/label-sync@v2`, whose latest tagged release (`v2.3.3`, Feb 2024) still runs on Node 20. The upstream Node 24 update merged on 2026-04-10 (commit `5594ed7`, `EndBug/label-sync#316`) but no `v2.3.4` release tag has been cut, so the floating `@v2` tag still points at the Node-20 release. Pinned to commit `1cd399a63f3d27e0a890cec8d1c29efad3e5db3d` (HEAD of upstream `main` as of 2026-04-25, which carries the Node 24 update plus a couple of routine dependency bumps), matching the SHA-pinning pattern already used in `dependabot-changelog.yml`. This removes the only remaining Node-20 action across this repo's workflows ahead of the GitHub Actions Node-20 deprecation deadline. Re-evaluate to a release tag once upstream cuts `v2.3.4`.
